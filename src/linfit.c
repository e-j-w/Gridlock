...
void refitFilterLin(const parameters * p, const data * d, fit_results * fr, plot_data * pd, long double distance){
  int i,j;
  ...
  for (i=0;i<d->lines;i++){
    long double diff = fabs(d->x[p->numVar][i] - evalLin(d->x[0][i],fr));
    if(diff<=distance){
