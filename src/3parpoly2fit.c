...
void refitFilter3Par(const parameters *, const data *, fit_results *, plot_data *, long double);
...
void plotForm3Par(const parameters * p, fit_results * fr, const plot_data * pd)
{
	...
	if(strcmp(p->plotMode,"1d")==0)
		{
			sprintf(fr->fitForm[0], "%.7LE*(x**2) + %.7LE*(%.7LE**2) + %.7LE*(%.7LE**2) + %.7LE*x*%.7LE + %.7LE*x*%.7LE + %.7LE*%.7LE*%.7LE + %.7LE*x + %.7LE*%.7LE + %.7LE*%.7LE + %.7LE",fr->a[0],fr->a[1],pd->fixedParVal[1],fr->a[2],pd->fixedParVal[2],fr->a[3],pd->fixedParVal[1],fr->a[4],pd->fixedParVal[2],fr->a[5],pd->fixedParVal[1],pd->fixedParVal[2],fr->a[6],fr->a[7],pd->fixedParVal[1],fr->a[8],pd->fixedParVal[2],fr->a[9]);
			sprintf(fr->fitForm[1], "%.7LE*(%.7LE**2) + %.7LE*(x**2) + %.7LE*(%.7LE**2) + %.7LE*x*%.7LE + %.7LE*%.7LE*%.7LE + %.7LE*x*%.7LE + %.7LE*x + %.7LE*%.7LE + %.7LE*%.7LE + %.7LE",fr->a[0],pd->fixedParVal[0],fr->a[1],fr->a[2],pd->fixedParVal[2],fr->a[3],pd->fixedParVal[0],fr->a[4],pd->fixedParVal[0],pd->fixedParVal[2],fr->a[5],pd->fixedParVal[2],fr->a[7],fr->a[6],pd->fixedParVal[0],fr->a[8],pd->fixedParVal[2],fr->a[9]);
			sprintf(fr->fitForm[2], "%.7LE*(%.7LE**2) + %.7LE*(%.7LE**2) + %.7LE*(x**2) + %.7LE*%.7LE*%.7LE + %.7LE*x*%.7LE + %.7LE*x*%.7LE + %.7LE*x + %.7LE*%.7LE + %.7LE*%.7LE + %.7LE",fr->a[0],pd->fixedParVal[0],fr->a[1],pd->fixedParVal[1],fr->a[2],fr->a[3],pd->fixedParVal[0],pd->fixedParVal[1],fr->a[4],pd->fixedParVal[0],fr->a[5],pd->fixedParVal[1],fr->a[8],fr->a[6],pd->fixedParVal[0],fr->a[7],pd->fixedParVal[1],fr->a[9]);
    }
  else if(strcmp(p->plotMode,"2d")==0)
		{
			sprintf(fr->fitForm[0], "%.7LE*(%.7LE**2) + %.7LE*(x**2) + %.7LE*(y**2) + %.7LE*%.7LE*x + %.7LE*%.7LE*y + %.7LE*x*y + %.7LE*%.7LE + %.7LE*x + %.7LE*y + %.7LE",fr->a[0],pd->fixedParVal[0],fr->a[1],fr->a[2],fr->a[3],pd->fixedParVal[0],fr->a[4],pd->fixedParVal[0],fr->a[5],fr->a[6],pd->fixedParVal[0],fr->a[7],fr->a[8],fr->a[9]);//x=par[1],y=par[2]
			sprintf(fr->fitForm[1], "%.7LE*(x**2) + %.7LE*(%.7LE**2) + %.7LE*(y**2) + %.7LE*x*%.7LE + %.7LE*x*y + %.7LE*%.7LE*y + %.7LE*x + %.7LE*%.7LE + %.7LE*y + %.7LE",fr->a[0],fr->a[1],pd->fixedParVal[1],fr->a[2],fr->a[3],pd->fixedParVal[1],fr->a[4],fr->a[5],pd->fixedParVal[1],fr->a[6],fr->a[7],pd->fixedParVal[1],fr->a[8],fr->a[9]);//x=par[0],y=par[2]
			sprintf(fr->fitForm[2], "%.7LE*(x**2) + %.7LE*(y**2) + %.7LE*(%.7LE**2) + %.7LE*x*y + %.7LE*x*%.7LE + %.7LE*y*%.7LE + %.7LE*x + %.7LE*y + %.7LE*%.7LE + %.7LE",fr->a[0],fr->a[1],fr->a[2],pd->fixedParVal[2],fr->a[3],fr->a[4],pd->fixedParVal[2],fr->a[5],pd->fixedParVal[2],fr->a[6],fr->a[7],fr->a[8],pd->fixedParVal[2],fr->a[9]);//x=par[0],y=par[1]
		}
}
...
void fit3Par(const parameters * p, const data * d, fit_results * fr, plot_data * pd, int print)
{
  ...
  linEq.vector[9]=d->msum;
	...
	if(!(solve_lin_eq(&linEq)==1))
		{
			printf("ERROR: Could not determine fit parameters (3parpoly2).\n");
...
void refitFilter3Par(const parameters * p, const data * d, fit_results * fr, plot_data * pd, long double distance){
  int i,j;
  ...
  for (i=0;i<d->lines;i++){
    long double diff = fabs(d->x[p->numVar][i] - eval3Par(d->x[0][i],d->x[1][i],d->x[2][i],fr));
    if(diff<=distance){
